// Src: http://infochim.u-strasbg.fr/cgi-bin/libs/smilesDrawer-master/doc/Vector2.js.html
...
export default class Vector2 {
     ...
     */
    toStringPolarRads() {
        return '(' + this.length() + ',' + this.angle() + 'rad)';
    }
     ...
     * @returns {String} A string representation of this vector.
     */
    toStringPolarDegrees() {
        return '(' + this.length() + ',' + this.angle() * 180 / Math.PI + '°)';
    }
    ...
    /**
     * Rotates this vector by 90 degrees around the origin of the coordinate system.
     *
     * @param {Number} angle The angle in radians to rotate the vector.
     * @returns {Vector2} Returns itself.
     */
     ...
     * 
     * @returns This same vector, set to (0,0).
     */
    zero() {
        this.x = 0;
        this.y = 0;
     ...
     * Works the same as new Vector2(), but without creating a new Object.
     * 
     * @returns This same vector, set to (x,y).
     */
    set(x, y) {
        if (arguments.length == 0) {
            this.x = 0;
    ...
    //#region Custom code
    /** 
     * Creates an array of zero vectors... 
     * To be used as temporal vectors, to avoid object creation overhead,
     * though performance have not been tested. 
     * 
     * @returns {Vector2[]} Array of zero vectors.
     */
    ...
    /**
     * Checks if any of the vector components are NaN.
     * @returns {Boolean} Returns a boolean indicating if any of the vector components is NaN.
     */
    isNaN() {
        return isNaN(this.x) || isNaN(this.y);
    }
    /**
     * Checks if both vector components are finite.
     * @returns {Boolean} Returns a boolean indicating if both components are finite.
     */
    isFinite() {
        return isFinite(this.x) && isFinite(this.y);
    }
    /**
     * Maps the vector components according to a given function. 
     * Similar to Array.map().
     * 
     * @param {Function} fn Function to map to each component of the vector.
     * @returns {Vector2} Mapped vector.
     */
    map(fn) {
        let x = fn(this.x, this);
        let y = fn(this.y, this);
        this.x = x;
        this.y = y;
        return this;
    }
    /**
     * Offset this vector by x & y. Same as vec.add(new Vector(x, y)).
     * 
     * @param {Number} x Offset X value
     * @param {Number} y Offset y value
     * @returns {Vector2} Returns itself.
     */
    offset(x, y) {
        this.x += x;
    ...
    /**
     * Reciprocates this vector. (1/x, 1/y).
     *
     * @returns {Vector2} Returns itself.
     */
    reciprocate() {
    ...
    //#region Complex math
    /**
     * Divides this vector by another vector, as complex numbers.
     * 
     * @param {Vector2} v Vector to divide this with. 
     * @returns {Vector2} Returns itself.
     */
    complexDivision(v) {
    ...
    /**
     * Reciprocates this vector, as a complex number (1/z)
     * 
     * @return {Vector2} Returns itself.
     */
    complexReciprocate() {
        var scale = 1 / (this.x * this.x + this.y + this.y);
        this.x = +this.x * scale;
        this.y = -this.y * scale;
        return this;
    }
    ...
    static midpoint(vecA, vecB) {
        return new Vector2((vecA.x + vecB.x) * 0.5, (vecA.y + vecB.y) * 0.5);
    }
    /**
